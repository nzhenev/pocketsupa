import { introspectSupabaseSchema, SchemaIntrospection, TableInfo, EnumInfo } from "./introspection";
import fetch from "node-fetch"; // Для Bun будет работать встроенный fetch, импорт не нужен

type PBFieldType = "text" | "number" | "bool" | "select" | "relation" | "file" | "date";

// Mapping из psql типов в PB типы
function pgTypeToPB(type: string, isEnum: boolean | string = false): PBFieldType {
...
async function pbCollectionExists(pbUrl: string, token: string, name: string): Promise<boolean> {
  const r = await fetch(`${pbUrl}/api/collections/${encodeURIComponent(name)}`, {
...
async function pbCreateCollection(pbUrl: string, token: string, name: string, fields: any[], options: any = {}) {
  const body = {
    name,
    type: 'base',
    schema: fields,
  ...
  if (!resp.ok) {
    ...
    throw new Error(`PB create collection ${name} failed: ${resp.status} ${err}`);
  }
  return await resp.json();
}

// Основная функция: sync миграции → PB (создает только новые коллекции)
export async function syncSupabaseToPB(migrationsDir: string, pbUrl: string, adminToken: string) {
  ...
  for (const table of schema.tables) {
    ...
    const pbFields = table.columns
      ...
      .map(col => {
        ...
        if (col.references) return { ...columnToPBField({ ...col, type: "text" }, schema.enums), type: "relation", options: { collectionId: col.references.table } };
...
if (require.main === module) {
  ...
  if (!migrationsDir || !pbUrl || !adminToken) {
    console.error("Usage: bun run src/schema/syncToPB.ts <migrations_dir> <PB_URL> <ADMIN_TOKEN>");
    process.exit(2);
  }
  syncSupabaseToPB(migrationsDir, pbUrl, adminToken).then(r => {
